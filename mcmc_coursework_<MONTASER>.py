...
def sample_beta_0(y, x, beta_1, beta_2, tau, mu_0, tau_0):
    ...
    mean= tau_0*mu_0 + tau * sum(y- beta_1*x - beta_2*(x**2))
    mean/=precision
...
def sample_beta_1(y, x, beta_0, beta_2, tau, mu_1, tau_1):
    ...
    mean=tau_1* mu_1 + tau * sum(x*y - x*beta_0 - beta_2*x**3)
    mean/=precision
...
def sample_beta_2(y, x, beta_0, beta_1, tau, mu_2, tau_2):
    ...
    mean=tau_2*mu_2 + tau*sum(y*(x**2) - beta_0*(x**2) - beta_1*(x**3))
    mean/=precision
...
def sample_tau(y, x, beta_0, beta_1, beta_2, alpha, beta):
    N = len(y)
    alpha_new = alpha + N / 2
    a=(y - beta_0- beta_1*x - beta_2*(x**2))**2
    beta_new =beta + sum(a)/2
